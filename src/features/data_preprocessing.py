...
data_preprocessing = CustomLogger(logger_name='data_preprocessing',
                              log_filename=log_file_path)
# set the level of logging to INFO
data_preprocessing.set_log_level(level=logging.INFO)
...
def save_transformer(path,object):
    joblib.dump(value=object,
                filename=path)
    data_preprocessing.save_logs(msg=f'{object} successfully saved')
...
def train_preprocessor(data:pd.DataFrame):
    ...
    data_preprocessing.save_logs(msg="Column transformer created successfully")
    return preprocessor
...
def transform_output(target:pd.Series):
    ...
    data_preprocessing.save_logs(msg="Power transformer created successfully")
...
def main():
    ...
    for filename in sys.argv[1:]:
        ...
        if filename == 'train.csv':
            data_preprocessing.save_logs(msg="...........Preprocessing for training data started..........")
            ...
            data_preprocessing.save_logs(msg="outlier transformer created successfully and data transformed")
            ...
            X_trans = transform_data(transformer=preprocessor,
                                     data=X)
            data_preprocessing.save_logs(msg="preprocessor transformer created successfully and data transformed")
            ...
            data_preprocessing.save_logs(msg="output transformer created successfully and target data transformed")
            ...
            data_preprocessing.save_logs(msg="Dataframe successfully saved for train data")
            
        elif filename == 'val.csv':
            data_preprocessing.save_logs(msg="...........Preprocessing for validation data started..........")
            df = read_dataframe(complete_input_path)
            # make X and y
            X = df.drop(columns=TARGET)
            y = df[TARGET]
            ...
            df_without_outliers = transform_data(transformer=outlier_transformer,
                                                data=X)          
            data_preprocessing.save_logs(msg="outlier transformer loaded successfully and data transformed") 
            ...
            X_trans = transform_data(transformer=preprocessor,
                                    data=X)
            data_preprocessing.save_logs(msg="preprocessor transformer loaded successfully and data transformed")      
            ...
            y_trans = transform_data(transformer=output_transformer,
                                    data=y.values.reshape(-1,1))
            data_preprocessing.save_logs(msg="target transformer loaded successfully and data transformed")      
            # save the transformed output to the df
            X_trans['trip_duration'] = y_trans
            ...
            save_dataframe(dataframe=X_trans,
                        save_path=save_data_path / filename)
            data_preprocessing.save_logs(msg="Dataframe successfully saved for validation data")
            
        elif filename == 'test.csv':
            data_preprocessing.save_logs(msg="...........Preprocessing for test data started..........")
            df = read_dataframe(complete_input_path)
            # load the transfomer
            outlier_transformer = joblib.load(save_transformers_path / 'outliers.joblib')
            df_without_outliers = transform_data(transformer=outlier_transformer,
            ...
            save_dataframe(dataframe=X_trans,
                        save_path=save_data_path / filename)
            data_preprocessing.save_logs(msg="Dataframe successfully saved for test data")
